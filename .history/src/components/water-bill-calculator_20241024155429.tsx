...
export function WaterBillCalculatorComponent() {
  ...
  const calculateBill = (usage: number) => {
    ...
    setTotalUsage(usage)
    return totalCost / 100 // Convert cents to Rand
  }
  ...
  const handleMeterChange = (meterId: string) => {
    ...
    if (meter) {
      setPreviousReading(meter.currentReading)
    }
    setBill(null)
    setTierBreakdown([])
    setTotalUsage(null)
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (meter) {
      ...
      const totalBill = calculateBill(usage)
      setBill(totalBill)
        ...
        <CardContent className="mt-6">
          <form onSubmit={handleSubmit} className="space-y-6">
            <div className="space-y-2">
              ...
              <Select onValueChange={handleMeterChange} required>
                <SelectTrigger id="meterSelect" className="text-lg p-6">
            ...
            {previousReading !== null && (
              <motion.div
                ...
                className="bg-blue-100 p-4 rounded-lg flex items-center space-x-2"
              >
                <Info className="text-blue-500" />
                <p className="text-blue-800">
                  Previous reading: <span className="font-bold">{previousReading}</span>
                </p>
              </motion.div>
            )}
            <div className="space-y-2">
